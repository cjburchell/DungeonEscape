...
{
    public sealed class DungeonEscapeScreenFade : MonoBehaviour
    {
        private const float DefaultDuration = 0.2f;
        private static DungeonEscapeScreenFade instance;
        private Texture2D texture;
        private float alpha;

        public static DungeonEscapeScreenFade GetOrCreate()
        {
            if (instance != null)
            {
                return instance;
            ...
            instance = FindAnyObjectByType<DungeonEscapeScreenFade>();
            if (instance == null)
            {
                instance = new GameObject("DungeonEscapeScreenFade").AddComponent<DungeonEscapeScreenFade>();
            }
