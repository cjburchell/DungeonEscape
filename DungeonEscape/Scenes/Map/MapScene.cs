namespace Redpoint.DungeonEscape.Scenes.Map
{
    ...
    public class MapScene : Scene
    {
        ...
        public override void Initialize()
        {
            ...
            this._isOverWorld = map.Properties != null && map.Properties.ContainsKey("overworld") &&
                                bool.Parse(map.Properties["overworld"]);

            var songPath = map.Properties != null && map.Properties.ContainsKey("song")
                ? map.Properties["song"]
                : @"not-in-vain";
...

            {
                ...
                var tiledMapRenderer = tiledEntity.AddComponent(new TiledMapRenderer(map,
                    this._gameState.Party.HasShip && this._gameState.Party.CurrentMapIsOverWorld
                        ? new[] { "wall", "water2" }
                        : new[] { "wall", "water2", "water" }));
            ...
            if (this._start == null)
            {
                ...
                else if (this._gameState.Party.CurrentMapIsOverWorld &&
                         this._gameState.Party.OverWorldPosition != Vector2.Zero)
                {
            ...
            {
                var followerEntity = this.CreateEntity($"cart", spawn);
                followerEntity.AddComponent(new Cart(lastEntity, _player, this._gameState, renderOffset));
            }
        }
