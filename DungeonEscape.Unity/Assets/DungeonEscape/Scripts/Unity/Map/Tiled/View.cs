...
namespace Redpoint.DungeonEscape.Unity.Map.Tiled
{
    public sealed class View : MonoBehaviour
    {
        ...
        private LoadedMap currentMap;
        private string pooledMapAssetPath;
        private HashSet<int> blockedTiles = new HashSet<int>();
        private SpriteRendererPool rendererPool;
        private GameState gameState;
        private readonly Viewport viewport = new Viewport();
        private readonly List<NpcController> runtimeNpcs = new List<NpcController>();
        private readonly Dictionary<int, NpcController> occupiedNpcTiles = new Dictionary<int, NpcController>();
        ...
        private void Start()
        {
            ...
            rendererPool = new SpriteRendererPool(transform);
            RenderPreview();
        }
        ...
        public bool CanNpcMoveTo(int column, int row, NpcController npc)
        {
            EnsureMapLoaded();
        ...
        {
            get { return currentMap == null ? Loader.NormalizeMapId(mapAssetPath) : Loader.NormalizeMapId(currentMap.AssetPath); }
        }

        public void UpdateNpcTile(NpcController npc, int oldColumn, int oldRow, int newColumn, int newRow)
        {
            if (npc == null)
            {
                return;
        ...
        public void RefreshObjectState()
        {
            ...
            if (currentMap != null)
            {
                currentMap.BlockedTiles = Collision.BuildBlockedTiles(
                    currentMap.Root,
                    currentMap.Info,
                    mapWidth,
                    mapHeight,
                    gameState,
                    Loader.NormalizeMapId(currentMap.AssetPath));
        ...
        {
            mapAssetPath = Loader.NormalizeMapAssetPath(mapIdOrAssetPath);
        ...
        public bool TryGetWarpAt(WorldPosition position, out Warp warp)
        {
            EnsureMapLoaded();
            warp = null;
        ...
        public bool TryGetObjectAt(WorldPosition position, out TiledObjectInfo result)
        {
            EnsureMapLoaded();
            result = null;
            var mapId = currentMap == null ? null : Loader.NormalizeMapId(currentMap.AssetPath);
        ...
        {
            var loadedMap = Loader.Load(mapAssetPath);
            if (loadedMap == null)
            {
                return;
            }
            ...
            if (gameState != null)
            {
                gameState.InitializeMapObjects(Loader.NormalizeMapId(loadedMap.AssetPath), loadedMap.Info);
            }
            ...
            var spriteSets = TilesetSprites.LoadSpriteSets(tilesets, loadedMap.TileWidth, loadedMap.TileHeight);

            var canUseShip = CanUseShipOnMap(Loader.NormalizeMapId(loadedMap.AssetPath));
            if (loadedMap.BlockedTiles == null || loadedMap.BlockedTilesAllowShip != canUseShip)
            {
                loadedMap.BlockedTiles = Collision.BuildBlockedTiles(
                    loadedMap.Root,
                    loadedMap.Info,
                    mapWidth,
                    mapHeight,
                    gameState,
                    Loader.NormalizeMapId(loadedMap.AssetPath));
            ...
            var renderedSpriteCount = Renderer.RenderVisibleLayers(
                ...
                SettingsCache.Current.ShowHiddenObjects,
                Loader.NormalizeMapId(loadedMap.AssetPath),
                gameState,
        ...
        private static void PlayMapMusic(LoadedMap loadedMap, bool force)
        {
            ...
            if (!force && (SplashScreen.IsVisible || TitleMenu.IsOpen))
            {
                return;
            }
            ...
            {
                Audio.GetOrCreate().PlayMapMusic(song);
            }
        }
        ...
        private bool CanUseShipOnMap(string mapId)
        {
            return gameState != null &&
                   gameState.Party != null &&
                   gameState.Party.HasShip &&
                   string.Equals(Loader.NormalizeMapId(mapId), "overworld", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void SyncRuntimeNpcs(LoadedMap loadedMap, IList<TilesetSpriteSet> spriteSets)
        {
            ...
            if (runtimeNpcMapAssetPath != loadedMap.AssetPath || !HasLiveRuntimeNpcs())
            {
                ...
                foreach (var group in loadedMap.Info.ObjectGroups)
                {
                    foreach (var mapObject in group.Objects)
                    {
                        ...
                        if (!IsMapObjectVisible(Loader.NormalizeMapId(loadedMap.AssetPath), mapObject))
                        {
                            continue;
                        }
                        ...
                        var npc = npcObject.AddComponent<NpcController>();
                        npc.Configure(
                            this,
                            mapObject,
                            spriteSets,
