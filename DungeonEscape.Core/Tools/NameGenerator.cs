  ...
  public class NameGenerator
  {
    ...
    private class Chain
    {
      ...
      private (string, bool) select_link (string key) {
        ...
        var idx = Math.Floor(Random.NextDouble() * len);
      ...
      readonly Dictionary<string, Dictionary<string, double>> _chain = new Dictionary<string, Dictionary<string, double>>();

      public void Construct(List<string> data)
      {
        foreach (var names in data.Select(nameData => nameData.Split(new[] {" "}, StringSplitOptions.None)))
        {
    ...
    private readonly Dictionary<Gender, Chain> _chainCache = new Dictionary<Gender, Chain>();
